...
- **касса (v2 Этап 7; сплит прав 2026-07-16)** — `cash_accounts`/`cash_entries`:
  SELECT по праву `view_cash` ИЛИ `can_manage_cash`, запись (счета/операции) —
  только `can_manage_cash` (НЕ по подразделению; owner имеет оба по дефолту роли).
  Оба права кассы **выдаёт только owner** (`can_grant_cap`, owner-only ветка,
...
- **Сплит составных прав (2026-07-16, миграция `0004_split_capabilities`):**
  в `perm_overrides` теперь **15** настраиваемых прав — `edit_payments` (изменять
  платежи; UI правки пока нет, право держит UPDATE-политику БД) + `delete_payments`
  (удалять платежи, кнопка-корзинка); `create_users` (создавать сотрудников,
  ступенчатость как у управления) + `manage_users` (роли, права, деактивация,
  подразделение/должность); `view_cash` + `can_manage_cash` (выше). Явные
  переопределения старых прав скопированы в новые половинки бэкфиллом.
...
- **расходы (0009 → переделаны 0010–0016, 2026-07-27)** — таблица
  ...
  **UPDATE-политика есть с 0019** (2026-08-04, жалоба клиента «нельзя
  изменить комментарий и статью»): `expenses_update` — зеркало `_delete`
  (строка с делом — `manage_case_expenses` И видимость дела И (автор ИЛИ
  `can_manage_users`); строка фирмы — `can_manage_cash`). Гард
  `expenses_guard_update` держит НЕИЗМЕННЫМИ `case_id` (перенос между делом и
  фирмой = смена зоны видимости → только удалить и внести заново),
  `created_by` и `payroll_transaction_id`. Правка пересобирает строку кассы
  тем же триггером `cash_sync_on_expense`. Строки с
  `payroll_transaction_id` (авто-расход от выплаты ЗП) руками не создаются,
  не правятся и не удаляются — уходят вместе с выплатой каскадом. Композиция
  «право × видимость дела» сохранена: выдача права юристу НЕ открывает ему чужие дела.
...
include_before_opening bool (default false — 0019, см. ниже)`
...
— **`include_before_opening` (0019, 2026-08-04):** операции раньше
`cash_accounts.opening_date` в обороты и сальдо НЕ входят (их влияние уже сидит в
`opening_balance` — иначе деньги посчитались бы дважды). Флаг = точечное исключение
«эта операция в начальный остаток не вошла, считать её». Ставит только
`public.cash_entry_set_included(id, bool)` (SECURITY DEFINER под `can_manage_cash`:
отсечка бьёт и по авто-строкам, а их RLS на UPDATE не отдаёт). Триггеры
пересинхронизации переносят флаг на пересозданную строку (через
`delete … returning`, 0020 — иначе параллельная пометка терялась).
⚠️ **Условие `entry_date >= opening_date OR include_before_opening` живёт в
ШЕСТИ местах:** `entriesFromOpening` (lib/cash/saldo.ts), `manualOutflow` и
`cut` в `app/(app)/reports/cash/page.tsx`, `buildRegistryDoc`
(lib/reports/cash/documents.ts) + SQL `cash_balances_before` / `cash_turnover` /
`cash_flow_monthly` / `cash_cutoff_summary`. Разойдутся — журнал, реестр и
оборотка покажут разные деньги за один период (ревью 0019 нашло ровно это:
реестр операций отсечку не применял вовсе). Альтернатива для владельца —
перенести саму `opening_date` назад (кнопка в плашке), это правило сразу для
всех операций счёта, поэтому идёт **через подтверждение с цифрами**: сколько
операций начнёт считаться, на какую сумму и каким станет остаток.
— **Карточка операции (2026-08-04):** клик по строке журнала открывает модалку с
подробностями (счёт, статья, клиент, автор, остаток после) и правкой ПО ТИПУ строки:
ручная → форма кассы (`updateCashEntryAction`), приход платежа → форма платежа
(`edit_payments`), розхід расхода → форма расхода (0019). Детали тянутся лениво
(`getCashEntryDetails`) — журнал бывает на тысячи строк.
...
>
> **✅ РАЗУМНЫЕ ДАТЫ (2026-08-03, миграция 0018, НА ПРОДЕ).** Повод: платёж на
> 1 000 ₴ по делу «07/04/26» лежал с датой `0004-04-07` — при вводе года набрали
> 0004 вместо 2026. Дата формально валидна (существующий день существующего
> месяца), поэтому её пропустили и форма, и сервер, а в отчётах платёж провалился
> на две тысячи лет назад. Запись исправлена на `2026-04-07` (прод + локалка,
> суммы по делу не изменились).
> **Три барьера:** `<input type="date" min/max>` (`workDateBounds`/`birthDateBounds`),
> `isWorkDate`/`isBirthDate` во ВСЕХ server actions с датами (прежний
> `isValidDate` проверял только формат — он и пропустил), и CHECK-ограничения
> 0018 на 15 колонок (payments, expenses, cash_entries, cash_accounts, cases,
> case_acts, payment_plan_items, absences, payroll_transactions, clients, tasks).
> **Диапазоны:** рабочие даты 2000…сегодня+5 лет (будущее — под график
> рассрочки), дата рождения 1900…сегодня. В CHECK границы шире и фиксированы
> (2000…2100): выражение обязано быть IMMUTABLE, `current_date` там запрещён —
> точную верхнюю границу держит сервер.

> **🟡 КАРТОЧКА ОПЕРАЦИИ КАССЫ (2026-08-04, миграция 0019, ЛОКАЛЬНО — не на проде).**
> Жалоба клиента через владельца: (1) строки журнала кассы не кликабельны —
> «нужна модалка с подробной информацией»; (2) нельзя изменить комментарий,
> статью и прочее.
>
> **Корень второй жалобы был в БД, а не в UI:** у `public.expenses` не было
> UPDATE-политики вовсе (по замыслу 0009 «правка = удалить и внести заново»), а
> зеркала расходов — половина строк журнала. Опечатка в комментарии стоила
> удаления записи вместе с авторством и датой внесения. Миграция 0019 даёт
> `expenses_update` (зеркало `_delete`) + гард неизменяемых `case_id` /
> `created_by` / `payroll_transaction_id` (см. §5).
>
> **Плашка «операції датовані раніше дати початкового залишку» стала
> действующей.** Раньше она только объясняла, почему сумма не в оборотах.
> Теперь у владельца три исхода (его формулировка): перенести дату начального
> остатка счёта (кнопка в плашке — чинит все отсечённые операции разом),
> перенести дату самой операции (правка в её карточке) ИЛИ **внести операцию в
> оборот с её датой** — точечное исключение `cash_entries.include_before_opening`.
> ⚠️ Условие отсечки после этого живёт в ЧЕТЫРЁХ местах (TS + три SQL-функции),
> см. §5 `cash_entries`.
>
> **Карточка операции:** клик по строке → модалка «Операція» (факты + правка по
> типу строки + удаление ручной + действия для отсечённых). Правка расхода
> добавлена и во вкладку «Витрати», и в список расходов на карточке дела —
> иначе правка была бы в одном месте и отсутствовала в соседнем.
> Проверки: 239 unit + 164 integration (в т. ч. новый
> `tests/integration/cash-entry-edits.test.ts` — 14 тестов на политику, гарды и
> флаг) + `smoke:rls`. **Хвост: прод-миграцию 0019 не катали.**

> **✅ ОТЧЁТЫ КАССЫ (2026-08-03, миграция 0017, НА ПРОДЕ).**
> Требование клиента: «отчёты в кассе — и внутри программы, и выгрузкой (Excel,
> PDF)». Семь отчётов за ПРОИЗВОЛЬНЫЙ период (месяц/квартал/год/свой), все с
> выгрузкой в XLSX и печатной версией → PDF.
>
> **⚠️ ГЛАВНОЕ РЕШЕНИЕ: два источника денег, и это НАМЕРЕННО.** Разбор данных
> показал: платежей по делам 652 на 6,76 млн ₴ за все годы, а в кассе ОДНА
> операция — счёт открыт 27.07.2026, авто-приход работает только с этого дня.
> Кнопка «Синхронізувати» не лечит: заводит операции датой платежа, а всё
> раньше `opening_date` из расчётов выпадает (считается сидящим в начальном
> остатке). Владелец выбрал **разделить**: «сколько заработали, от каких дел /
> клиентов / людей» → `public.payments` (вся история), «сколько на счетах, что
> пришло и ушло» → `public.cash_entries` (честно с даты открытия счёта). Суммы
> этих групп совпадать НЕ обязаны; UI подписывает это сноской, иначе читается
> как ошибка расчёта.
>
> **SQL (0017):** `cash_turnover(from,to)` и `cash_flow_monthly(from,to)` —
> DEFINER под `view_cash`/`can_manage_cash` (как `cash_balances_before`, с той же
> отсечкой по `opening_date`); `income_breakdown(from,to,dim)` — SECURITY
> **INVOKER** на `payments`: RLS `payments_select_via_case` скоупит строки по
> видимости дела сама, обходить её незачем. Разрезы: case|client|lawyer|expert|
> method|account.
>
> **Код:** `lib/reports/period.ts` (период + пресеты, `?from=&to=`, старый
> `?month=` жив), `lib/reports/export/` (ReportDoc → XLSX на exceljs),
> `lib/reports/cash/documents.ts` (7 сборщиков), роут выгрузки
> `/reports/cash/export`, печать `/reports/cash/print/[kind]` в route-group
> `(print)` — сегмент `/print/` обязателен, иначе `[kind]` перехватывает
> `/export`. **Один ReportDoc на экран, Excel и PDF** — цифры не могут разойтись.
> PDF = `window.print()`, без новых зависимостей (DOCX не делаем — решение
> владельца).
>
> **Выкачено 2026-08-03:** миграция накачена на прод-Neon ПЕРВОЙ, затем push в
> master (гоча типов дел 24.07). Сверка на проде: 777 оплат на 8 253 750 ₴,
> все разрезы сходятся с `finance_by_case`. **Хвост:** в данных есть платёж с
> датой `0004-04-07` (опечатка года) — чинить вручную.
...
- **Запреты прежние:** Inter/Roboto/system-ui и serif в display; градиенты на
  ...
  shadcn-slate; тёмная тема контента; infinite-анимации в списках.
  (Запрет hover-lift у строк 2026-06-07 снят 2026-07-23: строки списков —
  паттерн `.row-lift`, эталон «СРМ Вадима», см. DESIGN.md §5.)
