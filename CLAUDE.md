...
- **admin** (руководитель подразделения, контроль) — управляет пользователями и их
  правами, видит **все** дела и **все** финансы. **НЕ** трогает системные настройки.
- **office_manager** (офис-менеджер / секретарь) — заводит клиентов и дела, прикрепляет
  договоры, следит за порядком. Видит **все** дела и **все** финансы (ответ клиента).
  **НЕ** управляет пользователями и системными настройками; не удаляет записи и не
  правит платежи.
...
|---|---|---|---|---|---|
| Все дела | ✅ | ✅ | ✅ | ❌ (свои по `lawyer_id`) | ❌ (свои по `responsible_id`) |
| Финансы по всем делам | ✅ | ✅ | ✅ | по своим | по своим |
...
- `is_staff()` = `owner`/`admin`/`office_manager` → все строки и все финансы;
- `lawyer` → `cases.lawyer_id = auth.uid()`;
- `expert` → `cases.responsible_id = auth.uid()`;
- `can_manage_users()` = `owner`/`admin` → управление users + деструктив (DELETE,
  правка платежей);
- `is_owner()` = `owner` → системные настройки (`payroll_rates`);
- документы/задачи/платежи наследуют доступ от своего дела (`can_see_case` /
  `can_write_case`).
...
3. **Видимость дел:** юрист видит дела, где он `lawyer_id`; Експерт — где он
   `responsible_id`. Друг друга они не видят. Staff видит всё.
...
5. **admin и office_manager видят все финансы**; office_manager не удаляет записи и
   не правит платежи; управление пользователями — только owner/admin; системные
...
> бренд — **латунь / legal gold `#B88A3E`** (НЕ индиго), шрифты **Golos Text +
> JetBrains Mono** (НЕ Manrope/Geist Mono), золотой латунный градиент на CTA-кнопках
> и бренд-плашке. Пункты ниже про «индиго», «Manrope» и «нет градиентов на кнопках»
> ОТМЕНЕНЫ этой ревизией — актуальные значения см. в `DESIGN.md` и `globals.css`.
