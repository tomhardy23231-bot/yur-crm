...
Цель — интерфейс уровня продукта, а не «дефолтный CRUD». Это профессиональный рабочий
инструмент для юристов: им пользуются каждый день, в нём много данных и таблиц. Поэтому
«круто» здесь = **выверенно, быстро, плотно и доверительно**, а не пёстрый лендинг.
Refined-minimal, исполненный безупречно. Целься в уровень Linear / Vercel / Stripe Dashboard.

### Визуальное направление (зафиксировать через `/design-consultation`)
- Один чёткий характер: строгий editorial / refined, уверенный и спокойный.
- **Светлая и тёмная** темы с самого начала.
- Один насыщенный акцентный цвет + сдержанная нейтральная база. Палитру не размазывать.

### Жёсткие запреты (generic AI-look — не делать)
- НЕ использовать Inter, Roboto, Arial и системные шрифты как основные.
- НЕ фиолетовые градиенты на белом и прочие шаблонные схемы.
- НЕ предсказуемые «бутстрап-подобные» макеты и компоненты «из коробки».

### Типографика
- Характерная пара: выразительный шрифт для заголовков (напр. элегантный serif —
  Fraunces / Newsreader / Source Serif 4) + чистый гротеск для тела и данных
  (напр. Geist / Hanken Grotesk / IBM Plex Sans). Финал — за `/design-consultation`,
  но точно НЕ из запрещённого списка. Не больше 2 гарнитур.
- **Табличные цифры** (`font-variant-numeric: tabular-nums`) для всех денег, дат, счётчиков.

### Цвет и токены
- Все цвета — через CSS-переменные / дизайн-токены (никаких хардкодов в компонентах).
- Семантические токены: этапы воронки, приоритет (срочное), статус оплаты/долга.

### Плотность и макет (это data-tool)
- Информационно плотно, но читаемо: продуманные отступы, выравнивание по сетке.
- Сильные таблицы: липкие заголовки, сортировка, фильтры, числа выровнены вправо.
- Карточка дела — главный экран: секции (данные · документы · задачи · финансы · история)
  без перегруза.
- Командная палитра (Cmd/Ctrl-K) и горячие клавиши — инструмент для ежедневной работы.

### Движение (сдержанно и по делу)
- Один аккуратный staggered-вход при загрузке экрана; плавные переходы состояний.
- Никаких декоративных анимаций в рабочих сценариях — скорость важнее эффектов.

### Состояния (именно это отличает «полированный» продукт)
- Продуманные empty-state, skeleton-загрузка, аккуратные ошибки и подтверждения.

### Компоненты
- База — shadcn/ui, но **кастомизировать под токены**, чтобы не выглядело как дефолтный shadcn.

### Контроль качества дизайна (обязательно)
- Перед версткой нового экрана → `/design-consultation` (зафиксировать визуальную систему).
- После верстки → `/design-review` (полировка) + `/qa` (поведение в реальном браузере).
- Доступность (контраст, фокус, клавиатура) и производительность обязательны (Baseline 2026).
