...
export const caseCard = {
  ...
  detail: {
    ...
    roleExpert: 'Эксперт',
    // Совмещение ролей (0007): юрист и эксперт — один человек, начисление одно.
    roleDual: 'Юрист и эксперт',
  ...
  notFound: {
    ...
    description:
      'Дело не найдено или у вас нет к нему доступа. Возможно, его ведёт другой сотрудник. Если считаете, что это ошибка — обратитесь к руководителю.',
    goToMyCases: 'Перейти к моим делам',
  },

  // ── Форма дела (case-form.tsx) ───────────────────────────────────────
  form: {
    // Секции (подсказки — редизайн формы 14.07: понятно, что заполнять)
    sectionBasic: 'Основное',
    sectionBasicHint: 'Как называется дело, кто клиент и кто его ведёт.',
    sectionFinance: 'Финансы',
    sectionFinanceHint: 'Сумма договора, тип оплаты и расчёт вознаграждения.',
    sectionCourt: 'Судебное (если применимо)',
    sectionCourtHint: 'Заполняйте, только если дело дошло до суда.',
    sectionExtra: 'Дополнительно',
    sectionExtraHint: 'Теги — для поиска и фильтров, через запятую.',
    ...
    rateOverrideHint:
      'Необязательно. Пусто → берётся ставка категории. Меняет только владелец/руководитель подразделения.',
    lawyerRate: '% юриста',
    expertRate: '% эксперта',
    // Совмещение ролей (0007): юрист = эксперт → один процент вместо двух.
    dualRate: '% при совмещении ролей',
    dualRateHint:
      'Юрист и эксперт — один человек: зарплата начисляется один раз. Пусто → большая из ставок ролей.',
  ...
  // ── Сайдбар-помощник формы дела (case-form-aside.tsx, 14.07) ──────────
  formAside: {
    ratesTitle: 'Вознаграждение по категории',
    ratesHint: 'Процент от оплат по делу — юристу / эксперту:',
    ratesFootnote:
      'Ставки меняет владелец (Настройки → Ставки). На конкретном деле процент можно переопределить в секции «Финансы».',
    rolesTitle: 'Кто есть кто',
    roleLawyerTitle: 'Юрист (договор)',
    roleLawyerText:
      'заключил договор, вносит платежи и следит за доплатами по делу.',
    roleExpertTitle: 'Эксперт (исполнитель)',
    roleExpertText: 'ведёт дело, выполняет работу и выписывает акты.',
    nextTitle: 'Что будет после создания',
    next1: 'Дело появится в списке и на доске.',
    next2: 'На карточке добавите задачи, платежи, документы и акты.',
    next3: 'Каждое изменение запишется в историю дела.',
  },
  ...
  actions: {
    ...
    stageBackwardForbidden:
      'Возврат на предыдущий этап разрешён только руководству или офису.',
    stageSkipForbidden:
  ...
  // ── Модалка «ставка при совмещении ролей» (0007): юрист = эксперт дела ──
  dualRate: {
    title: 'Совмещение ролей в деле',
    subtitle: '{name} — и юрист, и эксперт этого дела',
    body: 'Зарплата по такому делу начисляется один раз. Выберите ставку или введите свой процент. Пока ставка не назначена, применяется большая из ставок ролей.',
    presetLawyer: 'Ставка юриста — {percent}%',
    presetExpert: 'Ставка эксперта — {percent}%',
    inputLabel: 'Процент, %',
    save: 'Сохранить ставку',
    later: 'Позже',
  },
};
