...
export const payroll = {
  ...
  report: {
    ...
    // KPI-плитки над отчётом (редизайн v5).
    kpiBalance: 'К выплате всего',
    kpiFixed: 'Оклады',
    kpiEarned: 'Заработано с дел',
    kpiBonus: 'Премии',

    // Шапка секции таблицы сотрудников.
    employeesTitle: 'Сотрудники',
    employeesCount: { one: '{n} сотрудник', few: '{n} сотрудника', many: '{n} сотрудников' },
    totalLabel: 'Итого',
  ...
  settings: {
    ...
    // Карточка-пояснение «Как рассчитывается зарплата» (правила §7-4 CLAUDE.md).
    how: {
      title: 'Как рассчитывается зарплата',
      formulaIntro: 'Выплата сотруднику =',
      formulaFixed: 'оклад (если задан)',
      formulaPercent: 'оплаты по делу × ставка категории',
      formulaBonus: 'премии',
      pointRates:
        'Ставка зависит от категории дела и задаётся раздельно для юриста и эксперта.',
      pointFullPercent:
        'Полный процент получает каждый — и юрист, и эксперт дела. Если это один человек — начисление одно (ставка при совмещении).',
      pointBase:
        'База расчёта — фактически оплаченные суммы по делу, а не сумма договора.',
      pointModes:
        'Режим сотрудника: процент, оклад (процент не начисляется) или оклад + процент.',
    },
  },
