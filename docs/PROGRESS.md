...
_Снимок на 2026-05-27 (двенадцатая сессия — Phase 1 в master)._

- **Phase 1 MVP + фаза шлифовки УШЛИ В `master`** 🚢 — коммит `3a27aea` (58 files, +4812/−129).
- **Что сделано в Phase 1** (CLAUDE.md §8): auth + роли + RLS, клиенты, дела + воронка, задачи + календарь, документы + Storage, платежи + автопересчёт, activity_log, поиск (PostgREST + RPC `search_case_ids`), Cmd+K палитра, дедлайны на главной, журнал.
- **Все проверки зелёные на момент коммита:**
  - `npm run lint` ✓
  - `npx tsc --noEmit` ✓
  - `npm run build` ✓ (17 routes)
  - `npm run db:seed` ✓ (5 users + 2 cases + 1 task + 1 payment)
  - `npm run smoke:rls` ✓ (13 блоков, 9 проверок в блоке 13 включая CSO #1)
- **Стек** на момент завершения сессии: dev-server :3000 (PID 19064 с сессии 11), Docker Supabase healthy, БД свежепосеяна.

### Следующая сессия — на выбор

- **Phase 2 (CLAUDE.md §8):** шаблоны документов (автоподстановка договор/претензия/доверенность), инвойсы, `time_entries`, расширенная аналитика. Порядок пока открыт.
- **Опционально перед Phase 2:**
  - `/document-release` — обновить CHANGELOG + README под факт «Phase 1 отгружено».
  - GitHub remote + `git push -u origin master` (всё ещё нет remote).
  - `/retro` — ретроспектива по 12 сессиям.

### Долги Phase 1, сознательно отложенные

- **GitHub remote не подключён** — коммиты живут локально. По запросу подключим и опционально PR через `/ship`.
...
- **MIME validation по magic-bytes / antivirus** — Phase 2 (Phase 1 — extension blacklist `FORBIDDEN_EXT`).
- **Электронная подпись / интеграция с ЄДРСР / клиентский портал** — Phase 3 (CLAUDE.md §9).
