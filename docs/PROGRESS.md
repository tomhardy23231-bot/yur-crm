...
## Сессия 2026-08-04 — КАРТОЧКА ОПЕРАЦИИ КАССЫ + ПРАВКА РАСХОДОВ

> **Миграции цикла: `0019_cash_entry_edits.sql` + `0020_cash_sync_hardening.sql`.**
> Прод-порядок — **миграции ПЕРВЫМИ, потом push в master** (гоча типов дел 24.07
> и отчётов кассы 03.08).

**Повод — жалоба клиента через владельца.** Два пункта:
1. строки журнала кассы не кликабельны — «нужна модалка с подробной информацией»;
2. «нельзя редактировать, изменять комментарий, статью и многое другое».

Плюс владелец не понял жёлтую плашку «У цьому періоді 1 операцій на 408 ₴
датовані раніше 2026-08-03…» и попросил разобрать.

### Что показывала плашка (разбор для владельца)

Счёт «моно» открыт с датой начального остатка **03.08.2026** (187 824 ₴), а
ручная операция «відсотки» **+408 ₴ датирована 01.08** — раньше неё. Начальный
остаток это «фотография счёта на дату»: всё, что раньше, считается уже внутри
него, иначе 408 ₴ посчитались бы дважды. Поэтому операция видна в журнале,
но с прочерком в колонке остатка и вне оборотов.

**Решение владельца — дать все три исхода, а не один:**
| Исход | Где |
|---|---|
| перенести дату начального остатка счёта назад | кнопка прямо в плашке — чинит все отсечённые операции разом |
| перенести дату самой операции | правка в её карточке |
| **внести операцию в оборот с её датой** | кнопка в карточке (новый флаг) |

### Корень второй жалобы был в БД

У `public.expenses` **не было UPDATE-политики вовсе** — по замыслу 0009 «правка =
удалить и внести заново». А зеркала расходов — половина строк журнала кассы
(Реклама, Оренда, Офіс). Опечатка в комментарии стоила удаления записи вместе с
авторством и датой внесения. Одной модалкой это было не закрыть.

### Миграция 0019

- `expenses_update` — зеркало `expenses_delete`: строка с делом →
  `manage_case_expenses` И видимость дела И (автор ИЛИ `can_manage_users`);
  строка фирмы → `can_manage_cash`. Зарплатные авто-строки не правятся вовсе.
- Гард `expenses_guard_update`: `case_id` / `created_by` /
  `payroll_transaction_id` неизменны. **Дело меняться не может намеренно** —
  перенос расхода между делом и фирмой это смена ЗОНЫ ВИДИМОСТИ, он должен
  оставить в журнале две записи, а не одну правку.
- `cash_entries.include_before_opening` + RPC `cash_entry_set_included`
  (SECURITY DEFINER под `can_manage_cash`: отсечка бьёт и по авто-строкам, а их
  RLS на UPDATE не отдаёт — но флаг это одна служебная колонка, не деньги).
- Три SQL-функции пересобраны под флаг: `cash_balances_before`, `cash_turnover`,
  `cash_flow_monthly`. Плюс TS `entriesFromOpening`.
- Триггеры `cash_sync_on_payment` / `cash_sync_on_expense` переносят флаг на
  пересозданную строку — иначе правка комментария молча снимала бы исключение.
- `expense_updated` в allowlist журнала (весь прежний список скопирован целиком,
  как требует конвенция; `log_activity` взят из 0014 дословно).

> ⚠️ **Инвариант на будущее.** Условие
> `entry_date >= opening_date OR include_before_opening` живёт теперь в
> **ЧЕТЫРЁХ** местах: TS `lib/cash/saldo.ts` + три SQL-функции выше. Журнал и
> вкладки счетов считаются в TS, отчёты периода — в SQL. Разойдутся — экраны
> покажут разные деньги, и это не будет видно из тестов одного слоя.

### Код

- Экшены: `updateExpenseAction`, `updateCashEntryAction`,
  `setCashEntryIncludedAction`, `shiftAccountOpeningDateAction`,
  `loadCashEntryDetailsAction`; запрос `getCashEntryDetails` (ленивый — журнал
  бывает на тысячи строк).
- Формы вынесены из диалогов, чтобы карточка операции их переиспользовала, а не
  дублировала: `payment-edit-form.tsx`, `expense-edit-form.tsx`,
  `cash-entry-edit-form.tsx`.
- `cash-entry-details-dialog.tsx` — карточка: факты (счёт, статья, клиент,
  автор, остаток после) + правка **по типу строки** + удаление ручной + блок
  действий для отсечённых операций.
- Правка расхода добавлена ТАКЖЕ во вкладку «Витрати» и в список расходов на
  карточке дела: иначе она есть в одном месте и отсутствует в соседнем.

### Проверки

`npx tsc --noEmit` ✓, `npm run lint` ✓, 239 unit ✓, 164 integration ✓
(новый `tests/integration/cash-entry-edits.test.ts` — 14 тестов: матрица прав
правки, оба гарда, пересборка строки кассы, флаг в оборотах и переносе остатка,
сохранение флага при правке источника), `npm run smoke:rls` ✓.

**Поймал страж `tests/unit/activity-labels.test.ts`** (заведён 03.08): новое
действие `expense_updated` было в миграции, но без подписи в словарях — ровно
тот случай, ради которого его писали.

### Ревью перед выкаткой нашло блокер (и он был мой)

`/review` на RLS-изменения (правило CLAUDE.md §11) отработало не формально —
адверсариальный проход остановил выкатку. Главное, что нашли:

| Что | Почему опасно | Как починено |
|---|---|---|
| Кнопка «Перенести дату остатка» срабатывала **без подтверждения**, а подпись обещала «Начальный остаток не изменится» | Перенос даты назад заставляет считаться всё, что раньше лежало внутри остатка. Один клик после «Синхронізувати» (777 платежей датами оплат) раздул бы кассу на всю историю — порядка 8 млн ₴ | `ConfirmDialog` с ЦИФРАМИ: сколько операций начнёт считаться, на какую сумму, каким станет остаток; честное предупреждение про задвоение |
| Отсечённые операции считались **по видимому периоду**, а кнопка меняет настройку счёта навсегда | В августе предлагалась дата 02.08, хотя такие же операции лежали и в апреле | SQL `cash_cutoff_summary()` (0020) — сводка по ВСЕМУ счёту |
| Правка комментария у старого расхода **молча переносила деньги на другой счёт** | У расходов до 0015 конкретного счёта нет (только вид счёта). Селект подставлял первый счёт, сохранение записывало его | опция «Оставить как есть» (сохраняет `method`) + подстановка текущего счёта, если он деактивирован |
| Если счёт не резолвится, строка кассы оставалась со **старой суммой навсегда** | Расход 5 000, касса 500, ошибки нет, тост «Расход изменён». Путь стал достижим именно с 0019 | 0020: на UPDATE зеркало снимается — лучше нет строки, чем строка с неверной суммой |
| Перенос флага «внесено в оборот» — гонка read-then-delete | Параллельная пометка терялась, деньги выпадали из оборотов обратно | `delete … returning` в обоих триггерах (0020) |
| **Отсечка жила не в четырёх местах, а в пяти** | «Реєстр операцій» её не применял вовсе и уже расходился с оборотно-сальдовой — обе выгружаются в Excel и PDF | отсечка в `buildRegistryDoc` + сноска о скрытых строках прямо на документе; `manualOutflow` тоже |
| Кнопка «Редактировать» показывалась без прав на источник | Смотрящий с `view_cash` получал форму и отказ на сохранении | право считается по источнику теми же ветвями, что на сервере |
| Модалка мешала свежие детали со сталым пропсом страницы | Правка могла откатить чужое изменение суммы (у `cash_entries` нет версии строки) | шапка и формы читают свежий `details.entry` |
| Сбой загрузки деталей был невидим | Карточка вечно «Загрузка», предупреждение об отсечке молча исчезало | явная плашка ошибки |
| Отсечённые операции недоступны с телефона | Мобильный экран строится из разворота по дням, а их там нет | отдельный блок «Не входят в обороты» над днями |

Итог: **миграция 0020** (`0020_cash_sync_hardening.sql`) + правки UI.
`idempotency_key` платежа убран из полезной нагрузки карточки.

### Проверки после фиксов

`tsc` ✓, `eslint` ✓, **239 unit** ✓, **167 integration** ✓ (в т. ч. 17 в
`cash-entry-edits.test.ts`), `smoke:rls` ✓.

### Хвосты

- created_at строки кассы всё ещё обновляется при пересборке зеркала
  (правка расхода теряет «когда внесли» у строки КАССЫ, сам расход цел);
- у `cash_entries` / `expenses` нет версии строки — параллельная правка
  разрешается по принципу «кто последний, тот и прав» (у дел это решено
  `updated_at`, см. §5).

---

## Сессия 2026-08-03 (вечер) — КАРКАС КАССЫ «БУХГАЛТЕРИЯ» ✅ 🚀 НА ПРОДЕ

> **СТАТУС: выкачено.** Миграций НЕТ — только UI, i18n, тексты тура и справки
> плюс чистая функция расчёта. Коммиты `537c0c5` (демки) и `5161da9` (редизайн).

**Повод.** Владелец: «не особо нравится дизайн кассы, всё непонятно». Диагноз
по коду: после отчётов кассы (запись выше) в один ряд встали **11 пилюль** —
три счёта с суммами и восемь отчётов одинаковыми чипами, с переносом на две
строки; данные периода показывались дважды (разворот по дням + журнал ниже);
градиентная hero-полоса занимала верх экрана ради одного числа; кнопки
действий делили строку со вкладками.

**Как выбирали.** Собрал `docs/demos/cash-redesign.html` — один самодостаточный
файл с четырьмя экранами: «Зараз» (копия текущего, чтобы сравнивать не по
памяти) + три подхода — **A «Банк»** (карточки счетов + единая лента операций),
**B «Бухгалтерія»** (рейка разделов + плотные таблицы), **C «Дашборд»** (KPI,
график, топы). Токены цвета/шрифтов взяты из `globals.css`, данные вымышленные.
Владелец выбрал **B**.
...
| Было | Стало |
|---|---|
| 11 пилюль в ряд | рейка разделов слева (`components/cash/cash-rail.tsx`), три группы: **Рахунки** (счета + Зведення + Налаштування рахунків), **Гроші** (Доходи / Витрати / Прибутковість справ), **Звіти за період** (4 отчёта) |
| градиентная hero-полоса | белая полоса из 4 чисел: остаток на сегодня, надійшло, витрачено, чиста зміна (+ 5-я ячейка «на кінець періоду», когда смотрим прошлый период) |
| кнопка «Рахунки» в шапке | раздел рейки «Налаштування рахунків» — счета и их настройка в одном месте |
| в журнале только сумма операции | + **накопительный остаток** `→ 1 238 060,00` после каждой операции |

**Права не трогали:** группа, данные которой пользователь не видит, не
рендерится вовсе (как раньше не рендерились вкладки). RLS — без изменений.

### Решения, которые стоит помнить

- **`rollForwardEntries` (lib/cash/saldo.ts)** — остаток после каждой операции.
  Считается из тех же `entriesFromOpening`, что и разворот по дням, поэтому
  **последняя операция дня всегда равна `closing` этого дня** — инвариант
  закрыт юнит-тестом на контрольном примере ОЛІМП. Операции раньше
  `opening_date` в мапу не попадают → в журнале прочерк с пояснением
  (они и не входят в сальдо).
- **Порядок журнала оставлен прежним (старые сверху).** С накопительным
  остатком иначе нельзя: колонка читается сверху вниз, как в книге.
- **Раздел в URL `?tab=` через `history.replaceState`, а не `router.push`.**
  Ссылку переслать можно, но переключение раздела не перезапускает
  `Promise.all` из 11 запросов страницы. Плата: «назад» уходит со страницы,
  а не переключает разделы.
- **Суммы в рейке — только у счетов и «Зведення».** У «Доходів» деньги
  считаются по оплатам в делах, а не по обороту кассы (см. «два источника
  денег» в записи выше) — подписать им оборот значило бы соврать.
- **Переименованы разделы** (в колонке есть место): Оборотка →
  «Оборотно-сальдова», Динаміка → «Рух по місяцях», Підсумок → «Фінансовий
  підсумок», Реєстр → «Реєстр операцій».

### Хвосты, которые иначе тихо сломались бы

Тур по кассе (релиз 2.11) привязан к `data-tour`-якорям: якоря перенесены на
новые элементы, стороны подсказок `bottom` → `right` (рейка теперь слева),
тексты шагов `cashHero*`/`cashTabs*`/`cashAccounts*` переписаны — они дословно
описывали старый экран. Обновлены `/help/cash` (uk+ru), скелетон `loading.tsx`,
удалены мёртвые ключи `totalBalance`/`balanceAtMonthEnd`. В `cash-rail.tsx`
якоря тура ставятся ТОЛЬКО в десктопной копии списка: мобильная копия скрыта
`md:hidden`, и `querySelector` нашёл бы её первой.

**Проверки:** `tsc` ✓, `eslint` ✓, 239 unit-тестов ✓ (+4 новых), `npm run build` ✓
(`/reports/cash` остался динамическим — `useSearchParams` не потребовал Suspense).

---

## Сессия 2026-08-03 — ОТЧЁТЫ КАССЫ + запрет бессмысленных дат ✅ 🚀 НА ПРОДЕ

> **СТАТУС: всё выкачено.** Миграции `0017` и `0018` применены на прод-Neon
> (каждая — ДО пуша кода), код — 4 коммита в master.

### Коммиты сессии
| Коммит | Что |
|---|---|
| `ad12065` | **отчёты кассы**: 7 отчётов за период + выгрузка XLSX/PDF, подписи журнала, свёрнутая форма создания пользователя |
| `121d8a9` | пометка о выкатке в CLAUDE.md |
| `48d3bb7` | **запрет бессмысленных дат** (три барьера + миграция 0018) |
| `bd92caa` | запись про защиту дат в CLAUDE.md |

---

### 1. Отчёты кассы (запрос владельца: «отчёты — и в программе, и выгрузкой»)

Семь отчётов за **произвольный период** (месяц / квартал / год / свои даты),
каждый с выгрузкой в Excel и печатной версией → PDF. DOCX решили не делать
(для финансовых таблиц Word — слабейший формат; владелец согласился).

| Вкладка | Что показывает | Источник |
|---|---|---|
| Оборотка | остаток на начало → приход → расход → остаток на конец по счетам | касса |
| Динаміка | приход/расход по месяцам + столбиковая диаграмма | касса |
| Доходи | откуда пришли деньги: дела / клиенты / юристы / Експерти / способ / счёт | **платежи** |
| Витрати | по статьям (была помесячной → стала за период) | expenses |
| Підсумок | доход − расходы = результат с детализацией | платежи + expenses |
| За справами | прибыльность дел (была помесячной → стала за период) | finance_by_case |
| Реєстр | все движения по счетам — для сверки с банком | касса |

#### ⚠️ ГЛАВНОЕ РЕШЕНИЕ: два источника денег, и это НАМЕРЕННО

Разбор данных вскрыл разрыв: платежей по делам **652 на 6,76 млн ₴** за все
годы (на проде — 777 на 8,25 млн), а в кассе **одна операция** — счёт открыт
27.07.2026, и авто-приход работает только с этого дня. Кнопка «Синхронізувати»
это НЕ лечит: она заводит операции датой платежа, а всё раньше `opening_date`
из расчётов выпадает (считается сидящим в начальном остатке) — счётчик написал
бы «додано 652», а цифры не сдвинулись бы.

Владелец выбрал **разделить источники**: «сколько заработали, от каких дел и
людей» → `public.payments` (вся история), «сколько на счетах, что пришло и
ушло» → `public.cash_entries` (честно с даты открытия). Суммы этих групп
совпадать НЕ обязаны; на вкладке «Доходи» стоит сноска, иначе разница читается
как ошибка расчёта. Альтернативы (залить историю в кассу задним числом)
отклонены: расходов за прошлые годы в кассе никто не вёл — вышло бы «пришло
6,76 млн, ушло 0».

#### Миграция 0017
- `cash_turnover(from,to)`, `cash_flow_monthly(from,to)` — SECURITY **DEFINER**
  с гейтом `view_cash`/`can_manage_cash` внутри (как `cash_balances_before`),
  та же отсечка по `opening_date` — цифры обязаны сходиться с вкладкой сальдо;
- `income_breakdown(from,to,dim)` — SECURITY **INVOKER** на `payments`: RLS
  `payments_select_via_case` скоупит строки по видимости дела сама, обходить
  её незачем. Разрезы: case|client|lawyer|expert|method|account;
- индексы `cash_entries(entry_date)`, `payments(paid_at)`.

Проверено на живой БД: владелец видит 777 оплат / 8 253 750 ₴, юрист — только
свои 222 / 2 095 000 ₴; все разрезы сходятся между собой и с `finance_by_case`.

#### Код
- `lib/reports/period.ts` — период и пресеты, URL `?from=&to=`, старый `?month=`
  продолжает работать (на него ведут закладки). 39 тестов;
- `lib/reports/export/` — **ReportDoc** (шапка + колонки + строки + итоги) и
  XLSX-рендерер на exceljs. Суммы уходят ЧИСЛАМИ с денежным форматом, шапка
  закреплена, автофильтр. 9 тестов;
- `lib/reports/cash/documents.ts` — 7 сборщиков; **один ReportDoc на экран,
  Excel и печать** → цифры физически не могут разойтись;
- роут выгрузки `/reports/cash/export`, печать `/reports/cash/print/[kind]`.
  ⚠️ Сегмент `/print/` обязателен: без него динамический `[kind]` перехватывал
  бы `/export` — два маршрута на один путь Next не разрешает;
- PDF = `window.print()` через существующий route-group `(print)` — **ноль
  новых зависимостей**, кириллица нативная.

Мемоизация: `getCashReportData`, `getTurnoverData`, `getExpensesByCategory`,
`getProfitByCase`, `commonHeader` обёрнуты в React `cache()` — семь документов
на странице просили одни и те же данные по нескольку раз.

### 2. Замечания владельца по ходу сессии
- **журнал показывал сырые коды** (`expense_created` и ещё 9 действий книги
  операций) — добавлены богатые формулировки + fallback-подписи в оба словаря.
  Плюс **тест-страж** `activity-labels.test.ts`: читает allowlist из миграций и
  падает, если новое действие уехало без перевода;
- **форма создания сотрудника** занимала весь первый экран `/settings/users` —
  свёрнута под кнопку (`UserCreatePanel`, как панель счетов в кассе).

### 3. Запрет бессмысленных дат (миграция 0018)

Повод: платёж на 1 000 ₴ по делу «07/04/26» лежал с `paid_at = 0004-04-07` —
при вводе года набрали 0004 вместо 2026. Дата формально валидна (существующий
день существующего месяца), поэтому её пропустили и форма, и сервер
(`isValidDate` проверял ТОЛЬКО формат), а в отчётах платёж провалился на две
тысячи лет назад и выпал из любого периода.

**Три барьера:**
1. `<input type="date" min/max>` — `workDateBounds()` / `birthDateBounds()`,
   проставлены в 14 полях 12 форм;
2. `isWorkDate` / `isBirthDate` во ВСЕХ server actions с датами (платежи,
   правка платежа, график, расходы, касса, счета, дела, акты, отпуска, ЗП,
   дата рождения);
3. **CHECK-ограничения 0018** на 15 колонок — ловят любой путь записи: сырой
   SQL, импорт, будущий код, забывший валидатор.
...
(2000…2100), потому что выражение обязано быть IMMUTABLE — `current_date` там
запрещён; точную верхнюю границу держит сервер.

Битая запись исправлена на `2026-04-07` (прод + локалка; `paid_total` дела не
изменился — 15 000 ₴). Полный скан обеих баз: других битых дат нет нигде.
Проверено на живой БД: `0004-04-07`, `0202-01-01`, `9999-01-01` отклоняются,
нормальная дата проходит.

### 4. Проверки
`npx tsc --noEmit`, `npm run lint`, **235 unit-тестов** (было 214, добавлено 21),
`npm run build` — всё чистое. Браузерных проверок не делали (правило владельца);
SQL-часть прогнана на живых базах.

### 5. Хвосты на следующую сессию
Владелец их видел и отложил («пока нет»):
1. **Касса не заполняется.** За неделю с запуска в неё внесли только расходы
   (205 973 ₴), приходов ноль. Решить: заводить историю по-честному или принять,
   что касса ведётся с августа. Пока не решено — «Оборотка», «Динаміка» и
   «Реєстр» будут почти пустыми.
2. **Мёртвый код.** `cases.accrual_mode` пишется формой, но ни на что не влияет
   (триггер снят в v3 s12); `payroll_ledger` заморожен и вырезан из UI, таблица
   осталась. Либо вернуть, либо убрать.
