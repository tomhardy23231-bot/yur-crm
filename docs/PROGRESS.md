...
> термины — читай их через это соответствие:
> - `jurist` / `specialist` (роль) → **`expert`** (Эксперт-исполнитель) или **`lawyer`** (юрист-продажник) по контексту;
> - `assistant` (помощник) и `supervisor` / `supervisor_id` / `current_user_supervisor_id` → **удалены** (роли «помощник» и иерархии супервайзеров больше нет);
> - `specialist_type` → **удалено**.
>
> Финальная модель — **5 ролей**: `owner`, `admin`, `office_manager`, `lawyer`, `expert`
> (см. `CLAUDE.md` §4). Актуальные тестовые логины — в `scripts/seed.ts`
...
> она отражает состояние на момент каждой сессии.
