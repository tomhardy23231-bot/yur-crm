-- ============================================================================
-- 0014_expense_category_actions_log.sql — журнал: удаление статьи расхода и
-- смена её области применения (по делу / по фирме).
--
-- ЗАЧЕМ. Владелец (2026-07-26) попросил не только заводить статьи, но и
-- редактировать и удалять их — из справочника и прямо из формы расхода.
-- Удаление статьи с уже внесёнными расходами запрещает FK
-- case_expenses_category_id_fkey ON DELETE RESTRICT (в UI показываем понятный
-- отказ и предлагаем скрыть статью).
--
-- ⚠️ Оба списка действий скопированы ЦЕЛИКОМ из 0012 и дополнены двумя
-- значениями — иначе прод-миграция падает 23514 на исторических записях.
-- ============================================================================
...
begin
  ...
  if p_action not in (
    ...
    'expense_category_deleted', 'expense_category_scope_changed') then
    return;
  end if;
...
comment on function public.log_activity(p_entity_type text, p_entity_id uuid, p_action text, p_changes jsonb) is
  'Журнал. 2026-07-26 (0014): +expense_category_deleted / expense_category_scope_changed. Прежний allowlist сохранён ЦЕЛИКОМ — гоча 23514. SECURITY DEFINER, size cap 8 КБ.';
