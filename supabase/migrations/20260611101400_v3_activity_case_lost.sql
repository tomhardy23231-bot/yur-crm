-- Юр CRM — v3 Сессия 7 (Продукт), часть 2: действие журнала 'case_lost'.
--
-- Добавляем 'case_lost' — закрытие дела как «не заключили» (public.close_case_lost).
-- entity_type='case', не-delete → гейт case-scope (private.can_see_case), как
-- payment_created/act_paid/case_updated.
--
-- ⚠ ГОЧА allowlist (PLAN-V3 шапка, SQLSTATE 23514): пересоздаём CHECK + log_activity
-- ПОВЕРХ 20260611100600_v3_activity_actions → ОБЯЗАНЫ сохранить ВЕСЬ прежний allowlist
-- целиком. База мёрджа — 20260611100600 (payment_updated/act_deleted/payroll_payout
-- уже включены). Добавляем только одно новое действие. entity_type-список НЕ меняем.
...
comment on function public.log_activity(text, uuid, text, jsonb) is
  'v3 Сессия 7: + case_lost (entity_type case, case-scope гейт). Прежний allowlist '
  '(case/client/document/payment/task/payroll/user/department/comment/act, в т.ч. '
  'payment_updated/act_deleted/payroll_payout из Сессии 2) сохранён целиком (гоча '
  '23514). SECURITY DEFINER, size cap 8 КБ.';
