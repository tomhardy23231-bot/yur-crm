-- Юр CRM — v2 Этап 5 (Акты), часть 3: журнал действий по актам.
--
-- Добавляем действия act_created / act_paid (entity_type='case' → видны в истории
-- дела по существующей политике activity_log_select_visible). act_created пишет
-- серверное действие создания акта; act_paid — функция public.confirm_act_paid.
--
-- ⚠ ГОЧА allowlist (PLAN-V2, SQLSTATE 23514): пересоздаём CHECK + log_activity
-- ПОВЕРХ 20260610140000 → ОБЯЗАНЫ сохранить ВЕСЬ прежний allowlist. База мёрджа —
-- 20260610140000 (user_salary_changed уже включён). Добавляем только act_*.
-- entity_type-список НЕ меняем: акт логируется как 'case'.
...
comment on function public.log_activity(text, uuid, text, jsonb) is
  'v2 Этап 5: + act_created/act_paid (entity_type case). Прежний allowlist '
  '(case/client/document/payment/task/payroll/user/department/comment/act) сохранён '
  'целиком (гоча 23514). SECURITY DEFINER, size cap 8 КБ.';
