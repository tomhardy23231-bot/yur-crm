-- Юр CRM — v3 Сессия 2 (Журнал и целостность), часть 1: новые действия журнала.
--
-- Добавляем действия:
--   payment_updated — задел под журналирование правок платежей (RLS-правки —
--                     отдельная тема; из UI пока НЕ вызывается, но ветка allowlist
--                     и формата готовы заранее);
--   act_deleted     — удаление неоплаченного акта (deleteActAction);
--   payroll_payout  — выплата зарплаты (createPayoutAction; раньше маскировалась
--                     под payment_created).
-- Все три — entity_type='case' (видны в истории дела), не-delete → гейт case-scope
-- (private.can_see_case), как payment_created/act_paid.
--
-- ⚠ ГОЧА allowlist (PLAN-V3 шапка, SQLSTATE 23514): пересоздаём CHECK + log_activity
-- ПОВЕРХ 20260610170000 → ОБЯЗАНЫ сохранить ВЕСЬ прежний allowlist целиком. База
-- мёрджа — 20260610170000 (act_created/act_paid уже включены). Добавляем только три
-- новых действия. entity_type-список НЕ меняем.
...
comment on function public.log_activity(text, uuid, text, jsonb) is
  'v3 Сессия 2: + payment_updated/act_deleted/payroll_payout (entity_type case, case-scope '
  'гейт). Прежний allowlist (case/client/document/payment/task/payroll/user/department/'
  'comment/act) сохранён целиком (гоча 23514). SECURITY DEFINER, size cap 8 КБ.';
